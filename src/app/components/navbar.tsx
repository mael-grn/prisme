...
export default function Navbar({websiteIdOrDomain} : {websiteIdOrDomain: string}) {
    ...
    return (
        <nav className={"fixed top-0 right-0 z-999 flex justify-end"}>
            ...
            <button onClick={() => setDeveloped(!developed)} className={`p-2 m-3 z-999 h-fit w-fit rounded-full ${developed && "scale-0 translate-x-100"} bg-background md:hover:bg-onBackgroundHover active:bg-onBackgroundHover active:scale-90 cursor-pointer flex items-center justify-center`}>
                <img src={developed ? "/ico/close.svg" : "/ico/hamburger.svg"} alt={"hamburger"} className={`${scrolled ? "w-6 h-6" : "w-10 h-10"} invert`}/>
            </button>
                    ...
                    <motion.ul
                        ...
                        </button>
                        {
                            website?.pages.map((page, index) => (
                                <motion.li key={index}
                                    initial={{filter: "opacity(0)", transform: "translateX(50px)"}}
                                    animate={{filter: "opacity(1)", transform: "translateX(0px)"}}
                                    exit={{filter: "opacity(0)", transform: "translateX(50px)"}}
                                    transition={{duration: 0.3, delay: index * 0.1}}
                                    onClick={() => router.push("/" + websiteIdOrDomain + "/" + page.path)}
